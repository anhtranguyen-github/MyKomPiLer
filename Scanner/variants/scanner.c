...
Token* readIdentKeyword(void) {
  ...
  // Collect characters for identifier or keyword
  while (length < MAX_IDENT_LEN) {
    if (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT || currentChar == '_') {
  ...
  // Skip remaining valid characters beyond MAX_IDENT_LEN
  while (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT || currentChar == '_') {
    readChar();
  }

  token->string[length] = '\0';

  // Check if the collected string matches a keyword
...
Token* getToken(void) {
  ...
  case CHAR_PLUS:
      ...
      if (charCodes[currentChar] == CHAR_EQ) {
          token->tokenType = SB_PLUS_ASSIGN; // "+=" token
          readChar();
          return token;
      }
      return token;
  ...
  case CHAR_TIMES:
      ...
      if (charCodes[currentChar] == CHAR_EQ) {
          token->tokenType = SB_TIMES_ASSIGN; // "*=" token
          readChar();
          return token;
      }
      return token;
  ...
  case CHAR_LT:
      ...
      if (charCodes[currentChar] == CHAR_EQ) {
          token->tokenType = SB_LE; // "<=" token
          readChar();
          return token;
      } else if (charCodes[currentChar] == CHAR_GT) {
          token->tokenType = SB_NEQ; // "<>" token
          readChar();
          return token;
      }
      return token;
